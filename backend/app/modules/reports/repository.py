...
class ProjectReportRepository:
    ...
    async def get_by_id(self, report_id: int) -> ProjectReport | None:
        statement = select(ProjectReport).where(ProjectReport.id == report_id)
        result = await self.db.execute(statement)
        return result.scalar_one_or_none()

    async def list_all(self) -> list[ProjectReport]:
        statement = select(ProjectReport).order_by(
    ...
    async def list_by_status(self, status: ReportStatus) -> list[ProjectReport]:
        statement = (
    ...
    async def list_by_project_for_author(self, project_id: int) -> list[ProjectReport]:
        statement = (
            select(ProjectReport)
    ...
    async def update(self, report: ProjectReport, data: ProjectReportUpdate) -> ProjectReport:
    ...
    async def submit(self, report: ProjectReport) -> ProjectReport:
